<script>
    ...
    function handleMouseDown(event) {
        ...
        canvasElements = [
            ...canvasElements,
            { id: newId, type: "shape", data: newShapeData, x, y },
        ];
    }
    ...
    function handleDrop(event) {
        ...
        try {
            ...
            canvasElements = [
                ...canvasElements,
                { id: Date.now(), type, data: finalData, x, y },
            ];
        } catch (e) {
...
>
        ...
        >
            ...
            <nav class="flex-1 w-full space-y-2">
                {#each sidebarItems as item, i}
                    <button
                        onclick={() => (openPannel = item.label)}
                        class={`flex flex-col items-center justify-center w-full py-3 hover:bg-gray-800 transition-colors ${openPannel === item.label ? "bg-gray-800 border-l-4 border-cyan-400" : ""}`}
                    >
    ...
    <main class="flex-1 flex flex-col min-w-0 bg-gray-100 relative">
        <!-- Top Bar -->
        <header
            class="h-14 bg-linear-to-r from-red-600 to-indigo-600 text-white flex items-center justify-between px-4 shadow-sm shrink-0"
        >
