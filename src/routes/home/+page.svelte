...
function handleDrop(event: DragEvent) {
    ...
    try {
        ...
        canvasElements = [...canvasElements, { id: Date.now(), type, x, y, ...data }];
    } catch (e) {
    ...
    <aside class="w-20 bg-gray-900 text-white flex flex-col items-center py-4 shrink-0">
        ...
        <nav class="flex-1 w-full space-y-2">
            {#each sidebarItems as item, i}
                <button onclick={() => openPanel = item.label} class={`flex flex-col items-center justify-center w-full py-3 hover:bg-gray-800 transition-colors ${openPanel === item.label ? 'bg-gray-800 border-l-4 border-cyan-400' : ''}`}>
