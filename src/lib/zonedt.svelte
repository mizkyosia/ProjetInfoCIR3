    ...
    //Ces valeurs peuvent être modifiées depuis le composant parent.
    export let x: number;   
    export let y: number;
    ...
    let isEditing = false;//Détermine si on affiche le texte simple ou le champ de saisie
    let showBorder = true;
    let isSelected = false;//Détermine si l'élément est cliqué
    let bgColor: string = "transparent";//Gère la couleur de fond.
    let container: HTMLElement;
    

    function startDragging(e: MouseEvent) {// permet le drag et drop 
        if (isEditing || (e.target as HTMLElement).closest('.word-toolbar')) return;
        //Si l'utilisateur est en train de taper du texte ou que Si l'utilisateur clique sur un bouton de la barre d'outils
        const startX = e.clientX - x;
        const startY = e.clientY - y;
        //Cette fonction interne s'exécute à chaque milliseconde dès que la souris bouge
        // elle recalcule la nouvelle position
    ...
    //
        function startRotating(e: MouseEvent) {//rotation de la zdt
            e.preventDefault(); e.stopPropagation();
            
            //trouve le contre de l'élement 
            const rect = container.getBoundingClientRect();
            const cx = rect.left + rect.width / 2;
            const cy = rect.top + rect.height / 2;
            //lle calcule l'angleentre l'axe horizontal et la ligne qui relie le centre
            const startAngle = Math.atan2(e.clientY - cy, e.clientX - cx) * (180 / Math.PI);
        
            const initialRotation = rotation;
            const onMouseMove = (e: MouseEvent) => {

            //ça recalcule le nouvel angle, fait la différene, on ajoute cette différence à la rotation initial
    ...
    function startResizing(e: MouseEvent) {//changement taille zone de texte
        e.preventDefault(); e.stopPropagation();
        //mémorise les dimensions de l'objet et la position du curseur
