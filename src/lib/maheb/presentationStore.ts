...
function loadPresentation(): Presentation {
    if (typeof window !== 'undefined') {
        ...
        return saved ? JSON.parse(saved) : defaultPresentation;
    }
...
function createPresentationStore() {
    ...
    const addSlideWithId = (id: number, label: string) => {
        update(pres => {
    ...
    const addButton = (slideId: number, label: string, color: string, destinationSlide: number) => {
        update(pres => {
    ...
    const deleteButton = (slideId: number, buttonId: string) => {
        update(pres => {
            const slide = pres.slides.find(s => s.id === slideId);
            if (slide) {
    ...
    const updateButton = (slideId: number, buttonId: string, updates: Partial<Button>) => {
        update(pres => {
            const slide = pres.slides.find(s => s.id === slideId);
            if (slide) {
